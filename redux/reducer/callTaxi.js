...
export default function callTaxi(preStatus = {
    ...
    switch (action.type) {
        ...
        case 'reverseGeoCodeResult':
            if (preStatus.locationWho === 'from') {
                var status = {...preStatus, from:{...preStatus.from, address: action.address, city:action.city, lng:action.lng, lat:action.lat, locationed: true}};
                return status;
            }
            break;
        ...
        case 'goAddrChanged':
            ...
            return {...preStatus, go:{...preStatus.go, locationed:true, address:goAddr.name, lng:goAddr.lng, lat:goAddr.lat}}
        default:
            return preStatus;
    }
